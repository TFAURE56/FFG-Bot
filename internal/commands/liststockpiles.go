...
func listStockpilesHandler(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	rows, err := db.Query("SELECT name, hexa, ville, style, code, cooldown FROM stockpiles")
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&sp.Nom, &sp.Hexa, &sp.Ville, &sp.Style, &sp.Code, &cooldownStr); err != nil {
	...
	// Réponse avec la liste des stockpiles ou message si aucun stockpile
	...
	for _, sp := range stockpiles {
		...
		if sp.Style == "normal" {
			emoji = "📦"
		} else if sp.Style == "bateau" {
			emoji = "⚓"
		} else if sp.Style == "avion" {
			emoji = "✈️"
		} else {
			emoji = "📦"
		}
		...
		response += fmt.Sprintf("# %s **%s** \n**🗺️ Hexagone: %s**\n**🌆 Ville: %s**\n🔑 Code: ||`%s`||\n⏳ Temps restant: %s\n🕒 Expire le: %s (heure de Paris)\n\n",
			emoji, sp.Nom, sp.Hexa, sp.Ville, sp.Code, cooldownStr, expirationParis)
	}

	if response == "" {
		response = "## 📦 Aucun stockpile enregistré pour ce serveur.\nUtilisez la commande `/addstockpile <nom> <hexa> <code>` pour en ajouter un.\n"
	} else {
